...
from complier.contract.ast import ModelPrompt, RetryPolicy
...
"""
        ...
        self.assertIsInstance(tool.guards[0], ModelPrompt)
        self.assertEqual(tool.guards[0].text, "must not contain harmful content")
...
"""
    ...
    def test_model_prompt_param_with_retry_policy(self) -> None:
        contract = Contract.from_source(
            """
workflow "research"
    | review gate=[must be safe and relevant]:2
"""
        )
        ...
        self.assertIsInstance(gate, ModelPrompt)
        self.assertIsInstance(gate.policy, RetryPolicy)
        self.assertEqual(gate.policy.attempts, 2)
        self.assertEqual(gate.text, "must be safe and relevant")
...
"""
    ...
    def test_guarantees_carry_text_and_retry_policy(self) -> None:
        contract = Contract.from_source(
            """
guarantee reviewed [must be safe and high quality]:2
...
"""
        ...
        self.assertIsInstance(expression, ModelPrompt)
        self.assertIsInstance(expression.policy, RetryPolicy)
        self.assertEqual(expression.policy.attempts, 2)
        self.assertEqual(expression.text, "must be safe and high quality")
...
"""
        ...
        for node in executable_nodes:
            ...
            self.assertIsInstance(node.guards[0], ModelPrompt)
            self.assertEqual(node.guards[0].text, "must not contain harmful content")
